...
module.exports = cds.service.impl((srv) => {
    ...
    srv.on(['CREATE'], ['Banks', 'Accounts', 'Customers'], async (req) => {
        let payloadKey = {};
        const targetname = req.target.name.split(".")[1], { bankID, accountid, custID } = req.data;
        if (targetname == 'Banks') {
    ...
    srv.on('Bank/Account/Customer/Created', async msg => {
        ...
        if (msg.data.KEY[0].entity == 'Banks') {
            ...
            const affectedrows = await tx.run(UPDATE(Accounts).set({ account_status: 'Active' }).where('accountid =', msg.data.KEY[0].ID));
            const sMessage = affectedrows ? `Status of Accounts Id ${msg.data.KEY[0].ID} updated successfully` : `Status of Accounts Id ${msg.data.KEY[0].ID} could not be updated`;
            ...
            const accountCount = await tx.run(SELECT.from(Accounts).where('customers_custID=', msg.data.KEY[0].ID))
